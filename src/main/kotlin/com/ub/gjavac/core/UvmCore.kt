...
import com.ub.gjavac.cecil.Instruction
...
class UvmProto {
    ...
    // .net IL指令到glua指令的映射
    val jvmInstructionsToGluaInstructionsMap: MutableMap<Instruction, UvmInstruction> = mutableMapOf()
    ...
    // 没有映射到glua instruction的.Net IL的nop等指令的列表，为了将每条IL指令关联到glua指令方便跳转查找，
    // 对于不产生glua instructions的IL指令，加入这个队列等待下一个有效非空glua instruction一起映射关联
    ...
    fun addInstruction(inst: UvmInstruction) {
        ...
        if (instJvmInstruction != null && !(inst is UvmEmptyInstruction)) {
            if (!jvmInstructionsToGluaInstructionsMap.containsKey(instJvmInstruction)) {
                jvmInstructionsToGluaInstructionsMap.put(instJvmInstruction, inst)
            }
            if (notMappedJvmInstructions.size > 0) {
                for (notMappedItem in notMappedJvmInstructions) {
                    jvmInstructionsToGluaInstructionsMap[notMappedItem] = inst
                }
    ...
    fun findUvmInstructionMappedByIlInstruction(ilInstruction: Instruction): UvmInstruction? {
        if (jvmInstructionsToGluaInstructionsMap.containsKey(ilInstruction)) {
            return jvmInstructionsToGluaInstructionsMap.get(ilInstruction)
        } else {
            return null
        }
    }
